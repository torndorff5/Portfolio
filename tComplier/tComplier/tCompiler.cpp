...
class compiler {
    ...
    vector<string> class_level_declarations;
    string class_scope = GLOBAL;
    const string DEFAULT = "-1";
...
public:
    ...
    //iCODE FUNCTIONS **************************************************************************************************************************
    string iCodeGen(string operation, string a_symid, string b_symid, string c_symid){
        string code = "\t" + operation + " " + a_symid + COMMA + " " + b_symid + COMMA + " " + c_symid + "\n";
        icode << code;
        return code;
    }
    string iCodeGen(string operation, string a_symid, string b_symid){
        string code = "\t" + operation + " " + a_symid + COMMA + " " + b_symid + "\n";
        icode << code;
        return code;
    }
    string iCodeGen(string operation, string label){
        string code = "\t" + operation + " " + label + "\n";
        icode << code;
        return code;
    }
    string iCodeGen(string label){
        string code = "\n" + label + ":";
        icode << code;
        return code;
    }
    ...
    void passop(token t){//#=
        ...
        if(class_scope == scope)
            class_level_declarations.push_back(iCodeGen(MOV, y.symid, x.symid));
        else
    ...
    void class_declaration(token&c, token&n){
        ...
        if(semantic){
            //declare default
            sym d = st->fetchSymbol(st->containsLexeme(DEFAULT, scope));
            __declareFunc(d.symid);
            for(auto s: class_level_declarations){
                icode << s;
            }
        ...
        if(syntax){
            ...
            d.scope = scope;
            d.kind = METHOD;
            d.symid = st->genSymID('d');
            d.value = DEFAULT;
            d.data.type = pop_tail_scope(scope);
            d.data.accessMod = PRIVATE;
            addSymbol(d);
        }
        pop_scope(scope);
    }
    ...
    void constructor_declaration(token& c, token& n){
        ...
        if(semantic){
            ...
            //call default
            sym d = st->fetchSymbol(st->containsLexeme(DEFAULT, scope));
            iCodeGen(FRAME, d.symid, THIS);
            iCodeGen(CALL, d.symid);
        }
        push_scope(scope,c.lexeme);
        getNextToken();
        if(c.type != parentho)
